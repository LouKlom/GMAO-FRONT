...
import ModifFabricantPopup from './ModifFabricantPopup'
import DeleteFabricantPopup from './DeleteFabricant'
import axios from 'axios';


export default function AdminFabricant() {

  const [isFabricantOpen, setFabricantOpen] = useState(false);
  const [isModifFabricantOpen, setModifFabricantOpen] = useState(false);
  const [isDeleteFabricantOpen, setDeleteFabricantOpen] = useState(false);
  ...
  const handleModifyClick = (id, nom) => {
    ...
    setSelectednom(nom);
    setModifFabricantOpen(true);
  }

  const handleDeleteClick = (id, nom) => {
    setSelectedItemId(id);
    setSelectednom(nom);
    setDeleteFabricantOpen(true);
  }
  ...
  return (
    <div>
      ...
      <div className="bg-gray-100 p-4 mx-10 mt-10">
            ...
            {isModifFabricantOpen && (
              <ModifFabricantPopup 
                onClose={() => setModifFabricantOpen(false)}
                selectedItemId={selectedItemId}
                selectedNom={selectedNom}
              />
            )}
            {isDeleteFabricantOpen && (
              <DeleteFabricantPopup 
                onClose={() => setDeleteFabricantOpen(false)}
                selectedItemId={selectedItemId}
                selectedNom={selectedNom}
              />
            )}
